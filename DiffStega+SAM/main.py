...
from segment_anything import SamAutomaticMaskGenerator, sam_model_registry  # Lily
...
class ODESolve:
    def __init__(self, args, EXT_SCALE=0., p=0.93, controlnet=None, current_mode="seg"):
        ...
        if controlnet is None:
            ldm_stable = StableDiffusionPipeline.from_pretrained("stable-diffusion-v1-5/stable-diffusion-v1-5",
                                                               safety_checker=None,
            ...
            ldm_stable = StableDiffusionControlNetPipeline.from_pretrained("stable-diffusion-v1-5/stable-diffusion-v1-5",
    ...
    for image_config in yaml_list:
        ...
        # seg
            if os.path.exists("{:s}/{:s}_seg.png".format(folder_path + "/seg",image_name)):
                ...
                mask_image.save("{:s}/{:s}_seg.png".format(folder_path + "/seg", image_name))
                seg_image = mask_image
                """
                只选取第一个 置信度分数最高的mask
